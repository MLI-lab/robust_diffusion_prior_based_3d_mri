...
class SparseUNet(nn.Module):
    def __init__(self, channels=3, nf=64, time_emb_dim=256, img_size=128, num_conv_blocks=3, knn_neighbours=3, uno_res=64, 
                 ...
                 uno_base_nf=64, kernel_interpolation_method = "bilinear"):
        super().__init__()
        ...
        for _ in range(num_conv_blocks):
            self.down_blocks.append(SparseConvResBlock(
                img_size, nf, kernel_size=kernel_size, mult=2, time_emb_dim=time_emb_dim, z_dim=z_dim, depthwise=depthwise_sparse, backend=backend, kernel_interpolation_method=kernel_interpolation_method
            ))
        ...
        for _ in range(num_conv_blocks):
            self.up_blocks.append(SparseConvResBlock(
                img_size, nf, kernel_size=kernel_size, mult=2, skip_dim=nf, time_emb_dim=time_emb_dim, z_dim=z_dim, depthwise=depthwise_sparse, backend=backend, kernel_interpolation_method=kernel_interpolation_method)
            )
    ...
    def forward(self, x, t, z=None, sample_lst=None, coords=None):
        ...
        # first check that t is a tuple
        if isinstance(t, tuple):
            sample_lst = t[1]
            height = t[2]
            width = t[3]
            t = t[0] # needs to be last
        ...
        # i.e. rearrange and set sample_lst.
            ...
            #sample_lst = torch.stack(
                ...
                #]).to(x.device) # sample lst has shape (B, size), e.g. 50 25600 -> (B, size, 1) -> (B, size, C)

            #x = rearrange(x, 'b c h w -> b (h w) c') # x has shape (B, 256*320, 2)
            #x = torch.gather(x, dim=1, index=sample_lst.unsqueeze(2).repeat(1,1,x.size(2))).contiguous()
      
        assert sample_lst is not None, "In sparse mode sample_lst must be provided"
        if coords is None:
            # this grid is used for the knn interpolation, and needs to match the grid used in the data (not the UNO)
            coords = torch.stack(torch.meshgrid(
                torch.linspace(0, 1, steps=height),
                torch.linspace(0, 1, steps=width)                )).to(x.device)
            coords = rearrange(coords, 'c h w -> () (h w) c')
            coords = repeat(coords, "() ... -> b ...", b=x.size(0))
