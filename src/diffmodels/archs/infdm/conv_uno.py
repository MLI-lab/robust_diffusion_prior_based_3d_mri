...
class FlashAttnBlock(nn.Module):
    def __init__(self, dim, min_heads=4, dim_head=32, mult=2, emb_dim=None, z_dim=None):
        ...
        from src.diffmodels.archs.std.unet import QKVAttention
        self.attn = QKVAttention(n_heads=num_heads)

    def forward(self, x, t=None, z=None):
        height = x.size(2)
        h = rearrange(x, "b c h w -> b (h w) c") # b x c -> b (3*num_head*x) c
        qkv = self.qkv(self.norm(h))

        qkv = rearrange(qkv, "b a c -> b c a")
        ...
        #h = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout=0.0)

        h = self.attn(qkv)

        h = rearrange(h, "b c a -> b a c")

        # Do Flash Attention
        #h, _ = self.attn(qkv)
        #h, _ = flash_attn_qkvpacked_func(qkv)
